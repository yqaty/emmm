...
requires KVTrait<Client, Slice *, Slice *> task<> run(Generator *gen, Client *cli, uint64_t cli_id, uint64_t coro_id)
{
    ...
    for (uint64_t i = 0; i < num_op; i++)
    {
        ...
        else if (op_frac < read_frac)
        {
            ...
            // {
            //     // log_err("[%lu:%lu]wrong value for key:%lu with value:%s expected:%s ret_value.len:%lu value.len:%lu", cli_id, coro_id, tmp_key,
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        bool rehash_flag = typeid(ClientType) == typeid(CLEVEL::Client) || typeid(ClientType) == typeid(ClevelFilter::Client) || typeid(ClientType) == typeid(ClevelSingleFilter::Client);;
        ...
        if(config.machine_id==0 && rehash_flag){
            ...
            // ClientType * rehash_cli = new ClientType(config, lmrs[config.num_cli * config.num_coro], rdma_clis[config.num_cli], rdma_conns[config.num_cli],rdma_wowait_conns[config.num_cli],config.machine_id, config.num_cli, config.num_coro);
        ...
        if(config.machine_id!=0 && rehash_flag){
            ...
            // lmrs[config.num_cli * config.num_coro] =
            //         dev.create_mr(cbuf_size, mem_buf + cbuf_size * (config.num_cli * config.num_coro));
            // ClientType* check_cli = new ClientType(config, lmrs[config.num_cli * config.num_coro], rdma_clis[config.num_cli], rdma_conns[config.num_cli],rdma_wowait_conns[config.num_cli],config.machine_id, config.num_cli, config.num_coro);
