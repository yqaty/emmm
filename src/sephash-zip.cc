...
task<> Client::reset_remote()
{
    ...
    for (uint64_t i = 0; i < (1 << dir->global_depth); i++)
    {
        ...
        dir->segs[i].local_depth = INIT_DEPTH;
        // log_err("dir->segs[i].cur_seg_ptr:%lx",dir->segs[i].cur_seg_ptr);
        co_await conn->write(dir->segs[i].cur_seg_ptr, seg_rmr.rkey, cur_seg, size_t(sizeof(CurSeg)), lmr->lkey);
    }

    // 重置远端 Directory
    // log_err("seg_rmr.raddr:%lx",seg_rmr.raddr);
...
task<uintptr_t> Client::check_gd(uint64_t segloc = -1)
{
    if(segloc!=-1){
        ...
        co_await std::move(read_cur_ptr);
...
task<> Client::insert(Slice *key, Slice *value)
{
    ...
    // co_await wo_wait_conn->write(kvblock_ptr, seg_rmr.rkey, kv_block, kvblock_len, lmr->lkey);
    retry_cnt = 0;
...
Retry:
    ...
    if(segptr == 0){
        ...
        // log_err("[%lu:%lu:%lu] zero segptr for segloc:%lu",cli_id,coro_id,this->key_num,segloc);
    ...
    if(key_num >= 1000){
        // log_err("[%lu:%lu:%lu] segloc:%lu seg_ptr:%lx",cli_id,coro_id,this->key_num,segloc,segptr);
    }

    // 2. read segment_meta && segment_slot concurrently
    // a. read meta
    CurSegMeta *seg_meta = (CurSegMeta *)alloc.alloc(sizeof(CurSegMeta));
    auto read_meta = conn->read(segptr + sizeof(uint64_t), seg_rmr.rkey, seg_meta, 4*sizeof(uint64_t), lmr->lkey);
    // b. read slots
    ...
    if(seg_meta->local_depth > dir->global_depth){
        ...
        if(seg_meta->local_depth > MAX_DEPTH){
            co_await sync_dir();
            dir->print();
            exit(-1);
        }
    ...
    // co_await wo_wait_conn->write(slot_ptr + sizeof(uint64_t), seg_rmr.rkey,
    //                             &tmp->fp_2, sizeof(uint8_t), lmr->lkey);
    ...
    if (seg_offset + slot_id == SLOT_PER_SEG - 1)
    {
        // Split
        // if(key_num >= 1000)
        //     log_err("[%lu:%lu:%lu] segloc:%lu depth:%lu",cli_id,coro_id,this->key_num,segloc,seg_meta->local_depth);
    ...
    // co_await conn->write(fp_ptr, seg_rmr.rkey,
    //                             &seg_meta->fp_bitmap[bit_loc], sizeof(uint64_t), lmr->lkey);
    perf.push_insert();
}
...
task<uint64_t> Client::filter_main_seg(Slot *seg,uint64_t len, Slot * filter_seg)
{
    uint64_t filter_len = 0;
    assert(len > 0 && len < MAX_MAIN_SIZE*2);
    KVBlock *kv_block_1 = (KVBlock *)alloc.alloc(130 * ALIGNED_SIZE);
    KVBlock *kv_block_2 = (KVBlock *)alloc.alloc(130 * ALIGNED_SIZE);
    filter_seg[filter_len++] = seg[0];
    for(uint64_t i = 1 ; i < len ; i++){
        if((seg[i-1].fp == seg[i].fp && seg[i-1].dep == seg[i].dep)){
            // 担心fp2没有正确更新，暂时不比较
            if((seg[i-1].fp_2 == seg[i].fp_2)){
                co_await conn->read(ralloc.ptr(seg[i-1].offset), seg_rmr.rkey, kv_block_1,seg[i-1].len * ALIGNED_SIZE, lmr->lkey);
                co_await conn->read(ralloc.ptr(seg[i].offset), seg_rmr.rkey, kv_block_2,seg[i].len * ALIGNED_SIZE, lmr->lkey);
                // kv_block_1->print("kv_1");
                // kv_block_2->print("kv_2");
                // 相同的fp，cur_seg中的数据会出现在更前面，保留前面的数据就行
                if(kv_block_1->k_len == kv_block_2->k_len && memcmp(kv_block_1->data,kv_block_2->data,kv_block_1->k_len) == 0){ 
                    continue;
                } 
            }
              
        }
        filter_seg[filter_len++] = seg[i];
    }
    co_return filter_len;
}
...
task<> Client::Split(uint64_t seg_loc, uintptr_t seg_ptr, CurSegMeta *old_seg_meta)
{
    ...
    // 2. Read MainSeg
    assert(cur_seg->seg_meta.main_seg_len == dir->segs[seg_loc].main_seg_len);
    ...
    // log_err("before merge_insert");
    // co_await print_segment(cur_seg->slots,SLOT_PER_SEG);
    // co_await print_segment(main_seg->slots, dir->segs[seg_loc].main_seg_len);
    merge_insert(cur_seg->slots, SLOT_PER_SEG, main_seg->slots, dir->segs[seg_loc].main_seg_len, new_main_seg->slots);
    // log_err("afer merge_insert");
    // co_await print_segment(new_main_seg->slots, dir->segs[seg_loc].main_seg_len+SLOT_PER_SEG);
    
    MainSeg *filter_new_main_seg = (MainSeg *)alloc.alloc(main_seg_size + sizeof(Slot) * SLOT_PER_SEG);
    uint64_t filter_len = co_await filter_main_seg(new_main_seg->slots, SLOT_PER_SEG + dir->segs[seg_loc].main_seg_len,filter_new_main_seg->slots);
    // MainSeg *filter_new_main_seg = new_main_seg;
    // uint64_t filter_len = SLOT_PER_SEG + dir->segs[seg_loc].main_seg_len;
    
    // log_err("afer filter");
    // co_await print_segment(filter_new_main_seg->slots, filter_len);
    FpInfo fp_info[MAX_FP_INFO] = {};
    // cal_fpinfo(new_main_seg->slots, SLOT_PER_SEG + dir->segs[seg_loc].main_seg_len, fp_info);
    cal_fpinfo(filter_new_main_seg->slots, filter_len, fp_info);
    ...
    if (dir->segs[seg_loc].main_seg_len >= MAX_MAIN_SIZE){
        ...
        for (uint64_t i = 0; i < filter_len ; i++)
        {
            dep_bit = (filter_new_main_seg->slots[i].dep >> dep_off) & 1;
            if (dep_off == 3)
            {
                // if dep_off == 3 (Have consumed all info in dep bits), read && construct new dep
                co_await conn->read(ralloc.ptr(filter_new_main_seg->slots[i].offset), seg_rmr.rkey, kv_block,
                                    filter_new_main_seg->slots[i].len * ALIGNED_SIZE, lmr->lkey);
                pattern = (uint64_t)hash(kv_block->data, kv_block->k_len);
                filter_new_main_seg->slots[i].dep = pattern >> (local_depth + 1);
                // if (kv_block->k_len != 8)
                // {
            ...
            if (dep_bit)
                new_seg_2->slots[off2++] = filter_new_main_seg->slots[i];
            else
                new_seg_1->slots[off1++] = filter_new_main_seg->slots[i];
        }
        ...
        // co_await wo_wait_conn->write(new_cur_seg->seg_meta.main_seg_ptr, seg_rmr.rkey, new_seg_2, sizeof(Slot) * off2, lmr->lkey);
        ...
        // co_await wo_wait_conn->write(cur_seg->seg_meta.main_seg_ptr, seg_rmr.rkey, new_seg_1, sizeof(Slot) * off1, lmr->lkey);
    ...
    uintptr_t new_main_ptr = ralloc.alloc(filter_len * SLOT_PER_SEG, true);
    uint64_t new_main_len = filter_len;
    // wo_wait_conn->pure_write(new_main_ptr, seg_rmr.rkey, new_main_seg->slots,
    //                             sizeof(Slot) * new_main_len, lmr->lkey);
    co_await wo_wait_conn->write(new_main_ptr, seg_rmr.rkey, filter_new_main_seg->slots,
                                sizeof(Slot) * new_main_len, lmr->lkey);
    ...
    cur_seg->seg_meta.main_seg_len = filter_len;
...
task<> Client::print_segment(Slot* slot,uint64_t slot_len){
    KVBlock *kv_block = (KVBlock *)alloc.alloc(130 * ALIGNED_SIZE);
    for (uint64_t i = 0; i < slot_len; i++){
        slot[i].print(i);
        co_await wo_wait_conn->read(ralloc.ptr(slot[i].offset), seg_rmr.rkey, kv_block,(slot[i].len) * ALIGNED_SIZE, lmr->lkey);
        std::string desc = "slot:"+std::to_string(i)+",slot_len:"+std::to_string(slot_len);
...
task<> Client::check_segloc(uint64_t seg_loc){
    uint64_t segptr = dir->segs[seg_loc].cur_seg_ptr;
    CurSegMeta *seg_meta = (CurSegMeta *)alloc.alloc(sizeof(CurSegMeta));
    co_await conn->read(segptr + sizeof(uint64_t), seg_rmr.rkey, seg_meta, 4*sizeof(uint64_t), lmr->lkey);
    if(seg_meta->local_depth > MAX_DEPTH){
        log_err("wrong local depth:%lu for segloc:%lu",seg_meta->local_depth,seg_loc);
        exit(-1);
    }
}
