...
namespace SPLIT_SEARCH_FPTABLE_WOCACHE
{
...
Retry:
    ...
    // 2.2 fptable
    uintptr_t fp_table_ptr = seg_rmr.raddr + sizeof(uint64_t) + segloc*sizeof(DirEntry) + 4 * sizeof(uint64_t);
    auto read_fp_table = wo_wait_conn->read(fp_table_ptr, seg_rmr.rkey, dir->segs[segloc].fp, MAX_FP_INFO*sizeof(FpInfo), lmr->lkey);
    

    // 3. Read  MainSlots
    co_await std::move(read_fp_table);
