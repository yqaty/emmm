...
void FilterCache::clear_filter(uint64_t level, uint64_t group_id, uint64_t buc_id)
{
    if (level == 1)
    {
        memset(sec_filter[group_id].filter[buc_id], 0, sizeof(uint64_t) * 4);
    }
    else
    {
        if (level == 2)
        ...
        memset(bot_filter[group_id].filter[buc_id], 0, sizeof(uint64_t) * 2);
    }
}
...
task<> Client::cal_utilization(){
    ...
    uint64_t dir_entry_consumption = 0;
    ...
    dir_entry_consumption += init_group_num * sizeof(TopPointer);
    ...
    // 3. 遍历第二层
    if(dir->cur_level>=1){
        space_consumption += sec_level_group_num * sizeof(SecGroupPointer);
        entry_total += init_group_num * bucket_per_group * entry_per_bucket;
        dir_entry_consumption += sec_level_group_num * sizeof(SecGroupPointer);
        for(uint64_t i = 0 ; i < sec_level_group_num ; i++){
            entry_cnt += dir->sec_level[i].size;
            for(uint64_t buc_id = 0 ; buc_id < bucket_per_group ; buc_id++){
                if(dir->sec_level[i].bucket_pointers[buc_id].buc_ptr != 0 ){
    ...
    // 3. 遍历后续层次
    uint64_t level_group_num = sec_level_group_num * fanout ;
    uint64_t level_id = 2;
    while(level_id <= dir->cur_level){
        space_consumption +=  sizeof(InnerGroupPointer) * level_group_num ;
        dir_entry_consumption += sizeof(InnerGroupPointer) * level_group_num;
        for(uint64_t group_id = 0 ; group_id < level_group_num ; group_id++){
    ...
    log_err("cur_level:%lu",dir->cur_level);
    log_err("space_consumption:%lu buc_meta_consumption:%lu dir_entry_consumption:%lu total_meta_consumption:%lu entry_total:%lu entry_cnt:%lu entry_utilization:%lf space_utilization:%lf",space_consumption,buc_meta_consumption, dir_entry_consumption, total_meta_consumption,entry_total,entry_cnt,entry_utilization,space_utilization);
    alloc.ReSet(0);
}
...
Retry:
    ...
    uint64_t *lock = (uint64_t*)alloc.alloc(sizeof(uint64_t),true);
    *lock = 0;
    ...
    co_await conn->read(group_ptr+sizeof(uint64_t), seg_rmr.rkey, &(top_group->epoch), sizeof(TopPointer)-sizeof(uint64_t), lmr->lkey);
    ...
    if (top_group->size[buc_id] >= entry_per_bucket)
    {
        ...
        *lock = 1;
        if(!co_await conn->cas(group_ptr, seg_rmr.rkey, *lock, 0)){
...
{
    // log_err("[%lu:%lu:%lu] migrate to next_level:%lu",cli_id,coro_id,*(uint64_t*)op_key->data,next_level);
    // 1. check cur_level
    uint64_t *cur_level = (uint64_t *)alloc.alloc(sizeof(uint64_t));
    co_await conn->read(seg_rmr.raddr, seg_rmr.rkey, cur_level, sizeof(uint64_t), lmr->lkey);
    ...
    for (uint64_t fanout_id = 0; fanout_id < fanout; fanout_id++)
    {
        ...
        while (free_buc_idx < bucket_per_group && elems_inserted < sizes[fanout_id])
        {
            ...
            for (uint64_t entry_id = 0; entry_id < elems_to_insert; entry_id++)
            {
                ...
                if (next_level == 1)
                {
                    auto [res_1, res_2] = cal_filter(tmp_hash);
                    sec_group_ptr->bucket_pointers[free_buc_idx].filter[0] |= res_1;
                    sec_group_ptr->bucket_pointers[free_buc_idx].filter[1] |= res_2;
                    auto [res_3, res_4] = cal_filter(tmp_hash, 2);
                    sec_group_ptr->bucket_pointers[free_buc_idx].filter[2] |= res_3;
                    sec_group_ptr->bucket_pointers[free_buc_idx].filter[3] |= res_4;
                    uint64_t filter_data[4] = {res_1, res_2, res_3, res_4};
                    filter_cache->update_filter(next_level, group_id, free_buc_idx, filter_data);
                }
                else
                {
                    auto [res_1, res_2] = cal_filter(tmp_hash);
                    inner_group_ptr->bucket_pointers[free_buc_idx].filter[0] |= res_1;
                    inner_group_ptr->bucket_pointers[free_buc_idx].filter[1] |= res_2;
                    uint64_t filter_data[2] = {res_1, res_2};
                    filter_cache->update_filter(next_level, group_id, free_buc_idx, filter_data);
                }
...
Retry:
    ...
    for (uint64_t i = 0; i < bucket_per_group; i++)
    {
        ...
        if (level == 1)
        {
            auto [res_1, res_2] = cal_filter(pattern);
            auto [res_3, res_4] = cal_filter(pattern, 2);
            bool result_1 = (res_1 & (!cache_filter[0])) == 0;
            bool result_2 = (res_2 & (!cache_filter[1])) == 0;
            bool result_3 = (res_3 & (!cache_filter[2])) == 0;
            bool result_4 = (res_4 & (!cache_filter[3])) == 0;
            filter_res = result_1 && result_2 && result_3 && result_4;
            // log_err("res_1:%lx res_2:%lx res_3:%lx res_4:%lx",res_1,res_2,res_3,res_4);
            // log_err("cache_filter[0]:%lx cache_filter[1]:%lx cache_filter[2]:%lx cache_filter[3]:%lx",cache_filter[0],cache_filter[1],cache_filter[2],cache_filter[3]);
            // log_err("result_1:%d result_2:%d result_3:%d result_4:%d",result_1,result_2,result_3,result_4);
        }
        else
        {
            auto [res_1, res_2] = cal_filter(pattern);
            bool result_1 = (res_1 & (!cache_filter[0])) == 0;
            bool result_2 = (res_2 & (!cache_filter[1])) == 0;
            filter_res = result_1 && result_2;
            // log_err("res_1:%lx res_2:%lx",res_1,res_2);
            // log_err("cache_filter[0]:%lx cache_filter[1]:%lx",cache_filter[0],cache_filter[1]);
            // log_err("result_1:%d result_2:%d",result_1,result_2);
        }

        // {
        //     uintptr_t buc_ptr = filter_cache->get_buc_ptr(level, group_id, i);
        //     log_err("buc:%lu buc_ptr:%lx",i,buc_ptr);
...
Retry:
    ...
    for (uint64_t i = 0; i < bucket_per_group; i++)
    {
        ...
        if (level == 1)
        {
            ...
            bool result_1 = (res_1 & (!remote_filter[0])) == 0;
            bool result_2 = (res_2 & (!remote_filter[1])) == 0;
            bool result_3 = (res_3 & (!remote_filter[2])) == 0;
            bool result_4 = (res_4 & (!remote_filter[3])) == 0;
            filter_res = result_1 && result_2 && result_3 && result_4;
            // log_err("res_1:%lx res_2:%lx res_3:%lx res_4:%lx",res_1,res_2,res_3,res_4);
            // log_err("remote_filter[0]:%lx remote_filter[1]:%lx remote_filter[2]:%lx remote_filter[3]:%lx",remote_filter[0],remote_filter[1],remote_filter[2],remote_filter[3]);
            // log_err("result_1:%d result_2:%d result_3:%d result_4:%d",result_1,result_2,result_3,result_4);
        ...
        else
        {
            ...
            filter_res = result_1 && result_2;
            // log_err("res_1:%lx res_2:%lx",res_1,res_2);
            // log_err("remote_filter[0]:%lx remote_filter[1]:%lx",remote_filter[0],remote_filter[1]);
            // log_err("result_1:%d result_2:%d",result_1,result_2);
