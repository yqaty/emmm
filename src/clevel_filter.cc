...
bool FilterCache::is_in_cache(uint64_t level_ptr, uint64_t buc_id){
    FilterID tmp{level_ptr,buc_id};
    return (cache.find(tmp) == cache.end());
}

void FilterCache::update_filter(uint64_t level_ptr, uint64_t buc_id,uint64_t* filter_data){
    FilterID cache_id{level_ptr,buc_id};
    if(is_in_cache(level_ptr,buc_id)){
        // 1. 判断是否在cache
        cache[cache_id].filter[0] |= filter_data[0];
        cache[cache_id].filter[1] |= filter_data[1];
    }else{
        // 2. 不在，判断空间是否够
        if(cache.size() < filter_cache_size){
            cache[cache_id] = {filter_data[0],filter_data[1]};
        }else{
            // 3. 空间不够判断是否需要淘汰old entry
            auto least_entry = cache.begin();
            if(least_entry->first < cache_id)
            {
                cache.erase(least_entry);
                cache[cache_id] = {filter_data[0],filter_data[1]};        
            }
        }
    }
...
uint64_t* FilterCache::get_filter(uint64_t level_ptr, uint64_t buc_id)
{
    auto iter = cache.find({level_ptr,buc_id});
    if(iter != cache.end()){
        return iter->second.filter;
    }
    return nullptr;
}
...
Retry:
    ...
    if(free_slot_ptr != -1){
        ...
        auto [res_1, res_2] = cal_filter(pattern_1);
        uint64_t filter_data[2] = {res_1,res_2};
        filter_cache.update_filter(free_level_ptr,free_buc_id,filter_data);
        bool filter_cas_1 = false;
        bool filter_cas_2 = false;
        while(filter_cas_1 == false || filter_cas_2 == false){
            // 因为造成的冲突，只能逐个进行cas了
            if(filter_cas_1==false){
                filter_cas_1 = co_await conn->cas(free_buc_ptr + sizeof(Entry)*BUCKET_SIZE, seg_rmr.rkey,   free_buc->filter[0] , free_buc->filter[0] | res_1);
            }
            if(filter_cas_2 == false){
                filter_cas_2 = co_await conn->cas(free_buc_ptr + sizeof(Entry)*BUCKET_SIZE + sizeof(uint64_t), seg_rmr.rkey,   free_buc->filter[1] , free_buc->filter[1] | res_2);
            }
        }
...
Retry:
    ...
    while(cur_table_ptr != 0){
        ...
        // a. 检查filter
        auto [res_1, res_2] = cal_filter(pattern_1);
        bool result_1 = (res_1 & (!buc1->filter[0])) == 0;
        bool result_2 = (res_2 & (!buc1->filter[1])) == 0;
        bool filter_res_1 = result_1 && result_2;
        bool result_3 = (res_1 & (!buc2->filter[0])) == 0;
        bool result_4 = (res_2 & (!buc2->filter[1])) == 0;
        bool filter_res_2 = result_1 && result_2;
